...
class DetailedTelegramCalendar(TelegramCalendar):
    ...
    def _build_days(self, *args, **kwargs):
        ...
        back_button = types.InlineKeyboardButton("🛑 Бекор қилиш", callback_data="back")
        ...
        days_buttons.append([{"text":back_button.text, "callback_data":back_button.callback_data}])
        ...
        for week in cl:
            for day in week:
                if day != 0 and self._valid_date(date(start.year, start.month, day)):
                    ...
                    ## start
                    ddd = datetime.datetime.strptime(str(date(start.year, start.month, day)), "%Y-%m-%d")
                    is_weekend = ddd.weekday() >= 6
                    if is_weekend or date(start.year, start.month, day) < today:
                        dates.append(None)
                    else:
                        ##end
